...
public interface ServiceMapper {
    ...
    @Update("update service set " +
            ...
            "work_time=#{workTime}, location=#{location}, price=#{price}," +
            "img_url=#{imgUrl}, link=#{link} where id=#{id}")
    public int update(Service service);

    // 插入服务数据
    @Insert("insert into service(sort_order, title, type," +
            "introduce, content_url, provider, work_time, location, price, img_url, link)" +
            "values(#{sortOrder}, #{title}, #{type}," +
            "#{introduce}, #{contentUrl}, #{provider}, #{workTime}, #{location}, #{price}, #{imgUrl}, #{link})")
